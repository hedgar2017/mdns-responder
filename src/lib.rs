...
impl Responder {
    fn setup_core() -> io::Result<(Core, Box<dyn Future<Item=(), Error=io::Error>>, Responder)> {
        let core = Core::new()?;
    ...
    pub fn with_handle(handle: &Handle) -> io::Result<(Responder, Box<dyn Future<Item=(), Error=io::Error>>)> {
        ...
        let (task, commands): (Box<dyn Future<Item=(), Error=io::Error>>, _) = match (v4, v6) {
            (Ok((v4_task, v4_command)), Ok((v6_task, v6_command))) => {
                let task = Box::new(v4_task.join(v6_task).map(|((),())| ()));
